...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", "https://crm.iacas.top", "https://*.iacas.top"],
...
if (process.env.NODE_ENV === 'production') {
  ...
  logger.info(`Servindo arquivos estáticos de: ${buildPath}`);
  
  // Verificar se o diretório build existe
  const fs = require('fs');
  if (fs.existsSync(buildPath)) {
    logger.info('Diretório build encontrado!');
    app.use(express.static(buildPath));
  } else {
    logger.error('ERRO: Diretório build não encontrado!');
  }
}
...
// Middleware de tratamento de erros ANTES da rota catch-all
app.use(errorHandler);

// Rota catch-all para React Router em produção (DEVE SER A ÚLTIMA ROTA)
if (process.env.NODE_ENV === 'production') {
  app.get('*', (req, res) => {
    const indexPath = path.join(__dirname, '../client/build/index.html');
    logger.info(`Requisição para: ${req.url} - Servindo: ${indexPath}`);
    
    const fs = require('fs');
    if (fs.existsSync(indexPath)) {
      res.sendFile(indexPath);
    } else {
      logger.error(`ERRO: index.html não encontrado em: ${indexPath}`);
